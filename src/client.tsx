...
type MenuItem = {
  ...
  modifier_groups: {
    name: string;
    type: string;
    modifiers: { name: string; price_delta: number; is_default: boolean }[];
  }[];
};
    ...
    <div className="space-y-3">
      {order.items.map((item, i) =>
        item.error ? (
          <Surface
            key={i}
            className="px-4 py-3 rounded-xl ring ring-red-500/20"
          >
          ...
          >
            <div className="px-4 py-2 border-b border-kumo-line bg-kumo-base flex items-center justify-between">
              ...
              <Text size="sm" bold className="text-kumo-accent">
                {cents(item.item_total)}
              </Text>
            </div>
              ...
              {item.modifiers.map((m, j) => (
                <div key={j} className="flex justify-between">
                  ...
                  <Text
                    size="xs"
                    variant="secondary"
                    className={m.price_delta > 0 ? "text-kumo-accent" : ""}
                  >
      ...
      <Surface className="px-4 py-3 rounded-xl ring ring-kumo-accent/30 bg-kumo-accent/5">
        <div className="flex justify-between items-center">
          ...
          <Text size="sm" bold className="text-kumo-accent">
            {cents(order.total)} {order.currency}
          </Text>
        </div>
      </Surface>
    </div>
...
function MenuView({ menu }: { menu: MenuItem[] }) {
  ...
  return (
    <div className="space-y-4">
      {categories.map((cat) => (
        <div key={cat}>
          <Text size="xs" bold variant="secondary" className="mb-2 block">
            {cat}
          </Text>
          <div className="space-y-2">
            {menu
              ...
              .map((p) => (
                <Surface
                  key={p.id}
                  className="px-4 py-3 rounded-xl ring ring-kumo-line"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <Text size="xs" variant="secondary" className="block">
                        {p.description}
                      </Text>
                    </div>
                    <Text size="sm" bold className="text-kumo-accent shrink-0 ml-3">
                      {cents(p.price)}
                    </Text>
                  </div>
    ...
    async (query: string) => {
      ...
      try {
        ...
        if (data.ok && data.type === "order" && data.order) {
          ...
          setMessages((prev) => [
            ...prev,
            { role: "agent", type: "menu", menu: data.menu as MenuItem[] },
          ]);
        } else {
          setMessages((prev) => [
            ...prev,
            { role: "error", text: (data.error as string) ?? "Unknown error" },
          ]);
        }
      } catch (err) {
        setMessages((prev) => [
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="secondary">
              <CoffeeIcon size={12} weight="bold" className="mr-1" />
              Agent
            </Badge>
          </div>
          <div className="flex items-center gap-2">
              ...
              <div key={i} className="flex justify-start">
                <div className="flex items-start gap-2 max-w-[90%] w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    {msg.type === "order" ? (
                      <OrderView order={msg.order} />
                    ) : (
                      <MenuView menu={msg.menu} />
                    )}
                  </div>
                </div>
              </div>
